...
def _valid_payload(**overrides):
    payload = {
        ...
        "end_date": "2026-09-10",
        "official_url": "https://example.com/edit-target",
        "source_name": "공식 출처",
        "summary": "요약",
    }
...
@pytest.mark.django_db
def test_get_renders_current_field_values(client, make_user, make_event):
...
@pytest.mark.django_db
def test_post_updates_event_and_writes_audit_log(client, make_user, make_event):
    staff = make_user(is_staff=True)
    client.force_login(staff)
    event = make_event(title="변경 전", official_url="https://example.com/edit-target")

    resp = client.post(_edit_url(event), _valid_payload())
...
def test_post_blank_title_returns_400_with_field_error_and_preserves_input(
    ...
    resp = client.post(
        _edit_url(event),
        _valid_payload(title="   ", official_url="https://example.com/blank-title-edit"),
    )
...
@pytest.mark.django_db
def test_post_blank_official_url_returns_400_with_field_error(client, make_user, make_event):
    staff = make_user(is_staff=True)
    client.force_login(staff)
    event = make_event(title="변경 전", official_url="https://example.com/blank-url-edit")

    resp = client.post(_edit_url(event), _valid_payload(official_url="   "))
...
@pytest.mark.django_db
def test_post_inverted_period_returns_400_with_field_error(client, make_user, make_event):
    staff = make_user(is_staff=True)
    client.force_login(staff)
...
def test_post_duplicate_official_url_returns_400_with_field_error(
    ...
    resp = client.post(
        _edit_url(event),
        _valid_payload(official_url="https://example.com/taken-by-other"),
    )
...
@pytest.mark.django_db
def test_post_allows_saving_with_unchanged_official_url(client, make_user, make_event):
    staff = make_user(is_staff=True)
    client.force_login(staff)
    ...
    resp = client.post(
        _edit_url(event),
        _valid_payload(title="변경 후", official_url="https://example.com/unchanged-url"),
    )
...
@pytest.mark.django_db
def test_list_row_links_to_edit_page(client, make_user, make_event):
    staff = make_user(is_staff=True)
    client.force_login(staff)
...
@pytest.mark.django_db
def test_get_form_action_preserves_list_filter_query(client, make_user, make_event):
...
@pytest.mark.django_db
def test_post_redirect_preserves_list_filter_query(client, make_user, make_event):
    staff = make_user(is_staff=True)
    client.force_login(staff)
    ...
    resp = client.post(
        ...
        _valid_payload(title="변경 후", official_url="https://example.com/prg-query"),
    )
