...
def test_planned_past_unvisited_is_auto_missed(django_user_model):
    user = _user(django_user_model, "d1")
    e = _event(date(2026, 6, 20))  # ended before today
    s = UserEventStatus.objects.create(user=user, event=e, status="planned")
    assert _derived(s) == "missed"


@pytest.mark.django_db
def test_planned_past_with_visit_is_not_missed(django_user_model):
    user = _user(django_user_model, "d2")
    e = _event(date(2026, 6, 20))
    s = UserEventStatus.objects.create(user=user, event=e, status="planned")
    VisitRecord.objects.create(user=user, event=e, visited_on=date(2026, 6, 19))
    assert _derived(s) == "planned"


@pytest.mark.django_db
def test_planned_past_overridden_stays_planned(django_user_model):
    user = _user(django_user_model, "d3")
    e = _event(date(2026, 6, 20))
    s = UserEventStatus.objects.create(
        user=user, event=e, status="planned", missed_overridden=True
    )
    assert _derived(s) == "planned"


@pytest.mark.django_db
def test_planned_future_is_planned(django_user_model):
...
@pytest.mark.django_db
def test_planned_boundary_today_is_not_missed(django_user_model):
...
@pytest.mark.django_db
def test_planned_null_end_date_is_planned(django_user_model):
...
@pytest.mark.django_db
def test_stored_visited_never_missed(django_user_model):
    user = _user(django_user_model, "d7")
    e = _event(date(2026, 6, 20))
    s = UserEventStatus.objects.create(user=user, event=e, status="visited")
    assert _derived(s) == "visited"


@pytest.mark.django_db
def test_stored_missed_before_date_is_missed(django_user_model):
    """Manually marking missed works even before the event ends."""
    user = _user(django_user_model, "d8")
    e = _event(date(2026, 6, 30))  # future
    s = UserEventStatus.objects.create(user=user, event=e, status="missed")
    assert _derived(s) == "missed"
...
@pytest.mark.django_db
def test_counts_move_past_planned_into_missed(django_user_model):
    user = _user(django_user_model, "c1")
    UserEventStatus.objects.create(
        user=user, event=_event(date(2026, 6, 20), title="past"), status="planned"
    )
    UserEventStatus.objects.create(
        user=user, event=_event(date(2026, 6, 30), title="future"), status="planned"
    )
...
@pytest.mark.django_db
def test_filter_missed_includes_auto_and_planned_excludes_it(django_user_model):
    user = _user(django_user_model, "c2")
    past = UserEventStatus.objects.create(
        user=user, event=_event(date(2026, 6, 20), title="past"), status="planned"
    )
    future = UserEventStatus.objects.create(
        user=user, event=_event(date(2026, 6, 30), title="future"), status="planned"
    )
...
@pytest.mark.django_db
def test_revert_to_planned_pins_via_override(django_user_model):
...
@pytest.mark.django_db
def test_mark_missed_before_date(django_user_model):
...
@pytest.mark.django_db
def test_mark_visited_removes_from_missed(django_user_model):
