...
| OAuth 활성화 | **코드 해소**(B2, 트랙 11) — 남은 조건은 사용자 액션 2건(아래 B2 참고) |
...
- 트랙 중 발견·즉시 수정: 소셜 가입 엔드포인트(`/accounts/3rdparty/signup/`)에
  ...
  Green(429) 왕복으로 검증.
- 남은 조건(코드 아님, 사용자 액션): ① Google Cloud Console에서 OAuth
  클라이언트 생성(리디렉션 URI `/accounts/google/login/callback/`), ②
  Render 환경변수에 `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` 설정. 활성화
  직후 실 OAuth 왕복으로 신규 유저가 동의 폼을 실제로 거치는지 수동 검증
  1회 필요.
