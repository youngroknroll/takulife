...
| F8 | 드래프트 상태 라벨이 4곳에 하드코딩 | `templates/core/drafts/list.html` 3곳 + `static/js/staff/draft_bulk.js`의 `STATUS_STATE_LABEL` | `[코드]` 현재 값은 일치. 갈라지면 낙관적 갱신과 서버 렌더가 다른 말을 한다 (PR #275 이월) |
