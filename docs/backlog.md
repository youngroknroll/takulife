...
기준일: 2026-07-31 · 기준 커밋: main `21820c0`
...
| 항목 | 값 |
|---|---|
| 백엔드 회귀 | `uv run pytest -q` → **2020 passed** (A1 전환으로 파라미터 34건이 가드 4개로 접혔다. 개수 감소 ≠ 커버리지 감소) |
...
**후속(트리거 있음)**: core 가드(같은 파일)도 손유지 목록 6개를 쓴다. 상대 임포트
수정이 머지된 지금은 글롭 전환이 안전하다 — 그전에는 오탐이 났다.
