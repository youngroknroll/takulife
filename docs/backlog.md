...
| 국내·미종료 필터(트랙 33) | `[실측 gh 2026-09-16]` PR #370 오픈(base `main`, 브랜치 `feat/domestic-active-draft-filter`) — 머지는 사용자 승인 대기. 러너·서버 양쪽 종료일·개최지 필터, 소스 후보 개최지 국가 규칙(`not_domestic`), 서버 수집 경로 한글·최신 날짜 필터, 승격 직후 초기 드래프트의 수집 규칙 결합, 실행 기록 `events_excluded` 저장. 마이그레이션 2개(`0010_sourcediscoveryrun_events_excluded`, `0011_alter_sourcecandidate_failure_stage`). 실기동 재현 완료(실행 14: 드래프트 2건 모두 국내, 제외 1건 저장) `[실측 2026-09-16]`. 기존 드래프트·소스 소급 정리는 범위 밖(로컬 해외 소스 2건은 비활성화 완료, 운영 DB는 수동 확인 필요). 회귀 수치는 PR #370 본문 참조. 가드레일 정본 `docs/BE/draft-source-agent-discovery.md` "트랙 33" 절 |
| 홈 이벤트 달력(트랙 34) | `[실측 gh 2026-09-17]` PR #373 오픈(base `main`, 브랜치 `feat/home-event-calendar`) — 머지는 사용자 승인 대기 — 홈 히어로 바로 아래에 월 격자(카테고리 점, 칸당 최대 4종)와 선택일 아젠다(최대 3건, 2026-09-17 사용자 지시로 5→3)를 서버 렌더로 표시, `?month=`·`?date=`는 행사 달력 파서 재사용·잘못된 값은 이번 달·오늘로 조용히 복귀, 56.25rem 1열·45rem 모바일, 다크는 토큰 상속. 같은 트랙에서 행사 달력 페이지의 선택일 포커스 이동(tabindex·scroll-margin)과 콘서트 색 누락도 수정. 홈 비로그인 GET 쿼리 7→8건 `[실측 CaptureQueriesContext]`. 신규 테스트 22건, 전체 회귀 2927 passed / 10 deselected `[실측 uv run pytest -q]`(HEAD `03a76e3a`). 가드레일 정본 `docs/FE/home-event-calendar.md`. 이연: 날짜 전개 규칙의 events 도메인 이관(트리거: 세 번째 호출부), `web/views/events.py` 분리(트리거: 700줄 초과, 현재 627줄 `[실측 wc -l]`), `.agenda-*` 스킨 공용화(트리거: 세 번째 소비처) |
...
### C2 [문서] 소개(`/about/`) 페이지 신설 — 트랙 28 후속, 별도 트랙 **착수 승인, 미착수(2026-09-10 사용자 결정)**

**범위 요약**: 공개 URL·뷰·템플릿 신설, `web/sitemaps.py`의 `StaticPageSitemap`
화이트리스트에 추가, `robots_meta` 블록의 noindex 해제, 본문 문안에 한글
브랜드명 '타쿠라이프' 2~3회 자연 노출.

**근거**: 트랙 28(PR #356)은 `<title>`·`<meta description>`·`WebSite` JSON-LD·
푸터 태그라인까지만 다뤘다. 본문 페이지 텍스트가 브랜드 검색 매칭의 가장 강한
신호라는 PSO 판정(`[문서] 2026-09-10`)에 따라 별도 트랙으로 분리했다.
...
| F17 | CI `e2e` 잡의 `continue-on-error: true` 해제 시점 | `[문서]` 트랙 27 DOR 제안 기준 — main 연속 10회 `[문서 DOR 제안]` 비결함 그린 뒤 필수 게이트로 전환 | 미해결 사용자 결정. 롤백은 잡에 `if: false` 한 줄 |
| F18 | 스태프 셸이 CSRF 쿠키를 새로 발급하지 않는다 | `[실측 2026-09-10 e2e E7]` `templates/staff/base_staff.html`은 `{% csrf_token %}` 0건이라 일반 셸(`templates/core/partials/_topbar.html:37` 로그아웃 폼)과 달리 `csrftoken` 쿠키를 심지 않는다. 로그인 폼을 거친 사용자는 쿠키가 있지만, 쿠키가 지워진 상태로 `/staff/events/`에 바로 들어오면 일괄 비공개·인라인 액션 fetch가 403("세션이 만료되었거나 보안 토큰…") | 후보 수정 = 스태프 목록 뷰에 `ensure_csrf_cookie` 또는 셸에 `{% csrf_token %}`. 트랙 27은 운영 코드 무변경 범위라 테스트 픽스처 `login_as`가 쿠키를 심는 것으로 대응했다 |
| F19 | 홈 스냅샷이 방문 기록 수를 활성 조건에 넣지 않는다 | `[코드 web/views/events.py:126-131, e2e E4에서 발견]` 방문 기록만 1건인 사용자에게도 홈 `snapshot_active`가 "아직 기록이 없어요"를 보여준다 | 제품 판단 필요(방문 기록 수를 활성 조건에 넣을지). 트랙 27은 E4의 Then을 방문 기록 목록으로 옮겨 대응했다 |
...
- `[코드]` `local_runner/page_fetch.py:129-147`(`_fetch_general_web`) 일반
  ...
  검사한다. 상한 자체가 메모리 사용을 막지 못한다(스트리밍 검사가 아님).
  개인 맥 단일 실행이라 위험이 낮아 이번 범위에서 뺐다.
- `[코드]` `local_runner/exploration_flow.py:80-81`(`run_exploration_flow`)
  탐색이 이벤트를 0건 낸 실행에서도 `client.known_urls(urls=[])`로 서버에
  빈 목록 조회를 보낸다. 결과를 바꾸지 않는 왕복이다.
- 검색 결과 페이지 자체를 발견 경로로 코드화하는 것(현재는 탐색 프롬프트가
  검색 결과 목록만으로 판단하게 지시할 뿐, 검색 결과 페이지 URL 패턴을
  코드가 알지 못한다). 트리거: 표본 주소가 검색으로 연속 두 번 안 잡히는
  사례 관측.
- 경로 정준화(같은 게시물의 다른 URL 형태를 하나로 묶기). 현재
  `_strip_tracking_params`(`drafts/agent_drafts.py:148-156`)는 추적
  쿼리스트링만 제거하고 경로 형식 차이는 다루지 않는다. 트리거: 경로
  형식만 다른 같은 행사가 별개 드래프트로 실제 등록되는 사례 관측.
- `[미검증]` 그 브라우저 제어 MCP 도구가 사용자의 상시 로그인 브라우저
  프로파일에 붙는지 격리된 새 프로파일을 쓰는지 확인하지 못했다(위 문서
  "격리 권고" 정정 절의 잔여 항목과 동일).
...
반려율 계산 기준(기산점·반려율 기준 모두 2026-09-08 확정, `docs/BE/draft-review-lifecycle.md` (c)·(i)). H10 — 반려
사유 입력 경로는 단건 반려의 textarea뿐
...
dash-metric 카드 재사용(집계만 백엔드). 구현됨(트랙 22, 왼쪽 열 `dash-panel`
+ `.dash-metrics--3` 변형 CSS, 링크 카드 포커스 링 인셋 규칙 추가). 두 역할 원문과 검증 기록은
...
- H2 — 체크박스 열 + bulkbar(`templates/core/drafts/list.html:50-61`) + 행별 실패
  ...
  체크박스 열·bulkbar·행별 실패 사유·목표 상태 지정 API는 **구현 완료
...
- H5 — `templates/core/drafts/detail.html:154-159` judge-actions에 재오픈 버튼,
  ...
  `successEl.focus()` 223) 재사용. 서버 pending 전용 가드로 멱등. 구현됨(트랙 21).
...
트랙 29(회원 닉네임 도입) 승인 범위 밖으로 이연한 항목. 가드레일 정본은
`docs/BE/account-identity.md`.

- 소셜 가입 시 provider가 주는 표시 이름을 닉네임 초기값으로 제안(현재는
  provider 값과 무관하게 빈 입력을 요구).
- 닉네임 변경 이력 보관(현재는 이전 값을 남기지 않는다).
