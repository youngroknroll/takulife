...
### D4 — 서비스 전체 공식 포스터 제거와 행사 에디토리얼 전환 **계획 승인**

2026-08-14 사용자 결정으로 공식 포스터 운영을 완전히 끝낸다. 공개 목록·달력·
홈·상세·활동·스태프 화면의 포스터와 대체 장식, `Event.poster_image`, 업로드 API,
포스터 누락 품질 경고와 저장된 목이미지가 제거 대상이다. 이미지는 보존하지 않는다.

행사 탐색은 작품명·행사명을 가장 강하게, 날짜를 같은 시선 흐름의 두 번째 축으로
표현하는 에디토리얼 인덱스로 바꾼다. 승인된 경험 계약과 범위는
`docs/FE/event-typography-editorial.md`, 구현 순서는
`.docs/FE/event-typography-editorial-plan.md`가 소유한다. **코드는 아직 미착수다.**
