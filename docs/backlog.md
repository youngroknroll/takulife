...
| 항목 | 값 |
|---|---|
| 백엔드 회귀 | `[실측 2026-09-09]` `uv run pytest -q` → **2474 passed**(73.21초, PR #346 머지 후 main) |
...
- H2 — 체크박스 열 + bulkbar(`templates/core/drafts/list.html:50-61`) + 행별 실패
  ...
  (2026-09-09, 트랙 24, PR #___)** — 정본 `docs/BE/staff-event-bulk-unpublish.md`.
  인라인 단건 토글 JSON 계약(트랙 25)과 정렬·기간 파라미터 확장(트랙 26)은
  남은 두 조각이다.
