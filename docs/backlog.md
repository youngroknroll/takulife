...
| 국내·미종료 필터(트랙 33) | `[실측 2026-09-16]` 브랜치 `feat/domestic-active-draft-filter`, 구현 완료·미머지·PR 미생성. 러너·서버 양쪽 종료일·개최지 필터, 소스 후보 개최지 국가 규칙, 서버 수집 경로 한글·최신 날짜 필터, 승격 직후 초기 드래프트의 수집 규칙 결합·제외 건수 저장까지 반영. 마이그레이션 2개(`0010_sourcediscoveryrun_events_excluded`, `0011_alter_sourcecandidate_failure_stage`). 전체 회귀 2900 passed / 10 deselected / 90.51초(기준선 2826 대비 +74)[실측], `manage.py check` 0 issues·`makemigrations --check --dry-run` 무변경[실측]. 실기동 재현은 아직 미확인. 가드레일 정본 `docs/BE/draft-source-agent-discovery.md` "트랙 33" 절 |
