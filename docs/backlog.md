...
| 행사 카탈로그 | `[실측 2026-08-01]` 게시 169건 중 **141건 종료(83%)**, 진행·예정 28건, 검증 완료 0건 |
...
**완료.** `core.analytics`를 `SERVICE_MODULES`에 넣고 공개 함수 4개를 전부 키워드
전용으로 바꿨다. **예외를 만들지 않았다**(2026-08-01 사용자 결정) — 모듈 단위
가드에 함수 단위 예외 목록이 생기면 그 근거가 계속 쌓인다.

검증: `uv run pytest -q` → **2145 passed**(파라미터 케이스 `core_analytics` +1),
check 0 issues, 드리프트 없음. **뮤테이션 왕복 4종 전부 Red** — 선언을 하나씩
위치 인자로 되돌릴 때마다 가드가 실패한다. 프로덕션 호출처가 실제 하중을 받는지도
`archive/services.py` 대표 3곳을 되돌려 확인했다(각 11·12·36건 실패).

⚠️ **착수 전 서술 두 개가 실측으로 뒤집혔다.**

- **"호출처 15곳 이상이라 A2보다 범위가 훨씬 크다" → 과대 서술.** 15곳이 맞지만
  **전부 이미 `record_event(EventName.X, user=...)` 형태**라 인자 재배열이 아니라
  `event_name=` 접두어 하나씩이었다. `distinct_user_key_count_since`와
  `event_name_counts_since`는 프로덕션·테스트 호출처 9곳이 **전부 이미 키워드
  형태**라 선언에 `*,`만 넣고 **호출처는 한 줄도 안 바뀌었다.**
  ★ 이 백로그가 낳은 **네 번째** 과대 서술이다(A3 8→2, A4 41→1, 경계 가드 34→18).
- **"`core.analytics`는 진짜 교차 도메인 서비스다"는 모듈 단위로만 참이다.**
  `pseudonymous_user_key`의 프로덕션 호출처는 `core/analytics.py:97` **모듈 내부
  하나뿐**이다 — 이 항목이 `accounts.services`를 기각한 논리("호출자가 전부 앱
  내부라 도메인 간 경계를 형성하지 않는다")가 이 함수에 그대로 들어맞는다.
  가드가 모듈 단위라 함께 걸리는 것이고, 그 예외를 만들지 않기로 한 것이 위 결정이다.

**"원 설계 의사 미확인"에 대해 코드가 준 단서**: `record_event(event_name, *, ...)`는
이미 `*`를 갖고 첫 인자만 밖에 있어 **누락이 아니라 의도**로 보였고(로깅 관용구),
반면 `distinct_user_key_count_since`는 `*`가 아예 없어 순수 누락이었다. 이 차이를
갈라 물었고 사용자가 예외 없는 쪽을 택했다.

★ **내 뮤테이션 도구가 먼저 거짓말을 했다.** 셸 이스케이프가 엉켜 치환이 아예
일어나지 않았는데 결과만 읽고 **4건 전부 "가드 무방비"로 오판**했다. 치환 발생
여부를 단언(`assert mut != base`)하도록 고치니 전부 Red였다. **뮤테이션 결과를
읽기 전에 뮤테이션이 실제로 걸렸는지부터 확인하라.**
...
### A3 [실측] 보유/구함/교환 3축 술어가 8곳에 흩어져 있고 뮤테이션에 걸리지 않는다 — **해결됨(2026-08-01, PR #277)**
...
### A4 [실측] 마크업 리터럴 단언 — **실제 위험은 41곳이 아니라 1곳이었다** — **해결됨(2026-07-30)**
...
#### 잔여 위험도 닫힘 (2026-08-01, PR #278)

`_boom`이 **실제로 호출됐는지** 단언해 닫았다. 몽키패치가 실행 경로를 벗어나면
호출 기록이 비어 단언이 실패한다.

**단, 위 「남은 위험」 서술은 불완전했다.** `[실측 2026-08-01]` `db` 픽스처만
떼는 것으로는 조용한 통과가 **재현되지 않는다** — 몽키패치가
`list_published_events`를 대체하므로 DB에 닿기 전에 `_boom`이 터진다. 조용한
통과는 **패치가 실행 경로를 벗어나는 것과 겹칠 때만** 일어난다.

| 조건 | 가드 이전 | 가드 이후 |
|---|---|---|
| 패치가 실행 경로를 벗어남 | RED | RED |
| **패치 벗어남 + `db` 없음** | **GREEN ← 조용히 통과** | **RED** |

★ **내가 이 판정에서 두 번 틀렸고 원인이 같다 — 대조군을 정확히 구성하지 않았다.**
①`db`만 떼고 GREEN이 나오자 "가드가 안 먹는다"고 읽었다(실제로는 그 조건이 결함을
만들지 않는다). ②HEAD판 템플릿에 **새 테스트를 함께 돌려 놓고** 그 실패를
"리팩터가 커버리지를 잃었다"는 근거로 삼았다 — 실패한 것은 새 가드였다.
**옛 코드를 되돌릴 때는 그 시점의 테스트도 함께 되돌려라.**
...
### B1 [코드] 직접 등록에 수정 기능이 없어 오타 수정이 파괴적 삭제를 강요한다 — **해결됨(2026-07-30, PR #255)**
...
**D1·D3 완료, D2 완료, D4 계획 승인 (2026-08-14).**
...
### D2 — **완료 (2026-08-01, 스태프 2개는 PR #275에서 닫힘)**

| 템플릿 | 칩 | 상태 |
|---|---|---|
| `core/events/list.html` | 7 | **완료** |
| `core/events/calendar.html` | 6 | **완료** |
| `core/drafts/list.html` | 4 | **완료** — `staff/views/drafts.py:78`이 렌더하는 **스태프 콘솔**이다(`templates/core/` 아래 있어 착각하기 쉽다) |
| `staff/events/list.html` | 3 | **완료** |

보류 사유였던 **콘솔 리디자인이 확정**되면서(PR #275) 재개 트리거가 충족됐고,
재설계가 바로 그 탭들을 다시 쓰면서 함께 닫았다. `[실측 2026-08-01]` 두 파일
모두 `aria-current="true"` + 가시 텍스트를 포함한 `aria-label="…, 적용됨"`을
갖는다(drafts 4개, staff/events 3개 — 착수 전 서술의 「2개」는 비활성 칩만 센 값).
...
→ `[실측 2026-08-01]` 리디자인(PR #275)을 통과해 **살아남았고**, 이제 그 속성을
단언하는 회귀 가드가 있다. 경고가 실제로 작동한 사례다.

### D4 [실측] 서비스 전체 공식 포스터 제거와 행사 에디토리얼 전환 — **해결됨(2026-08-15, PR #282)**

2026-08-14 사용자 결정으로 공식 포스터 운영을 완전히 끝냈다. 공개 목록·달력·
홈·상세·활동·스태프 화면의 포스터와 대체 장식, `Event.poster_image`, 업로드 API,
포스터 누락 품질 경고를 제거했고 저장된 목이미지 2,869개(14M) `[실측 2026-08-15]`를
백업 없이 영구 삭제했다. 사용자 사진(방문·직접등록 컬렉션)은 무손상 `[실측]`.

행사 탐색은 작품명·행사명을 가장 강하게, 날짜를 같은 시선 흐름의 두 번째 축으로
표현하는 에디토리얼 인덱스로 바뀌었다. 전체 백엔드 회귀 2144 passed `[실측]`,
FE 이중 게이트(Web Experience Designer·Browser Interaction Reviewer) 모두
`Conforms`. 승인된 경험 계약, 실측 증거와 남은 갭은
`docs/FE/event-typography-editorial.md`가 소유한다.
...
### E1 [코드] `core/views.py` 분할 — **해결됨(PR #251, 이 항목이 갱신되지 않았다)**

`[실측 2026-07-31]` `core/views.py`는 **존재하지 않는다.** `core/views/` 패키지로
분할됐다 — `account/activity/archive/collection/events/system/_helpers`.
⚠️ A1·A5에 이어 **세 번째로** 해결 뒤 표기가 갱신되지 않은 항목이다.

<!-- 아래는 착수 전 서술 (보존) -->
### E1 착수 전 기록 — `core/views.py`가 2189줄 / 최상위 함수 50개다
...
`[실측]` 1012줄 → **653줄**. 활동 달력 읽기 모델(상수 3개 + `CalendarActivityItem`
+ 함수 13개, 366줄)을 `archive/activity_calendar_queries.py`로 옮겼다.
**옛 모듈에서 재수출하지 않았다** — 별칭을 남기면 다음 사람이 어느 경로를 써야
할지 모른다. 소비자 3곳의 임포트를 전부 새 경로로 고쳤고, `CalendarActivityItem`과
kind 상수를 문자열로 언급하던 주석 2곳도 함께 정정했다(안 고치면 존재하지 않는
경로를 가리킨다).

<!-- 아래는 착수 전 서술 (보존) -->
### E2 착수 전 기록 — `archive/queries.py` **1012줄** — 활동 달력 읽기 모델이 함께 들어 있다
...
`[실측]` 1566줄 한 파일의 테스트 **79개**를 7개 파일로 나눴다. **물리적 위치가 아니라 어느 프로덕션 함수를 검증하는가로 묶었다** — 찜 구역에
있던 2건이 실제로는 `user_status_counts`를 단언해 상태 파일로 옮겨졌다.
"파일명 = 검증 계층"의 실제 의미가 그것이다.
AST 대조로 **79 → 79, 집합 동일, 본문 바이트 단위 동일** 확인. 세 커밋 각각
홀로 초록임도 확인했다.

⚠️ 착수 전 서술의 "5개 도메인"과 내가 센 "81개"는 둘 다 틀렸다 — 실제는
**7개 도메인 / 79개**다. 눈으로 센 값이었다.

<!-- 아래는 착수 전 서술 (보존) -->
### E3 착수 전 기록 — `tests/archive/test_archive_queries.py` **1566줄**에 5개 도메인이 섞여 있다

- 근거: 상태(38-90) · 방문기록(305-392) · 찜(443-707) · 직접등록(864-1007) ·
  컬렉션(1026-1528).
- 범위: 이 저장소가 이미 쓰는 "파일명=검증 계층" 관례를 쿼리 테스트에도 적용.

### E4 [실측] 주석 규칙 — **해결됨(2026-07-31, PR #269~#271)**

**범위 결정**: 사용자가 전면 적용을 택했다(2026-07-31). 적용 결과 314파일에서
주석 순 −3,757줄, 한국어 1,730 → 7,737줄, **영어 산문 0**.
`tests/core/test_comment_language_guard.py`가 재발을 막는다 — 한글이 없고 코드를
걷어낸 뒤에도 영어 문장이면 실패하며, 위반을 프로덕션 먼저 보고한다.
길이는 강제하지 않는다(2줄 초과가 44%이고 개조식 압축 여부를 기계가 못 본다).
⚠️ **가드가 스윕이 못 본 것을 찾아냈다** — 인라인 후행 주석 8건. 스윕 스캐너가
`#`로 시작하는 줄만 봐서 코드 뒤에 붙은 주석이라는 형태 자체를 놓쳤다.

<!-- 아래는 착수 전 서술 (보존) -->
### E4 착수 전 기록 — 주석 규칙이 저장소 전반에서 지켜지지 않는다
...
### E5 [실측] 주석이 조건을 대신 설명하는 자리 — **해결됨(2026-07-31, PR #272)**

6곳 전부 이름을 붙였다. **주석을 지우는 것이 목적이 아니었다** — 조건을 풀어 쓴
주석만 사라졌고, "왜 이 검사가 여기 있는가"(DB 제약으로 못 막아 2차 방어,
atomic 이전이어야 부분 저장 방지, 18자리는 bigint보다 한 자리 짧다 등)는 남겼다.
중복이던 pk 검사는 `core/query_params.py`의 `is_safe_pk_string`으로 합쳐졌다.
뮤테이션으로 술어 6개가 전부 하중을 받는 것을 확인했다(상수 고정 시 각각
19·23·19·5·73건 실패). 입력 5,015건 대조로 동작 불변 확인.
**나머지 33곳은 주석이 붙어 있지 않다** — 지금 누구도 헷갈리지 않는다는 뜻이라
일괄 처리 대상이 아니다. 해당 코드를 여는 작업에서 그때그때 판단한다.

<!-- 아래는 착수 전 서술 (보존) -->
### E5 착수 전 기록 — 주석이 조건을 대신 설명하는 자리

주석 정책은 **"코드가 먼저, 주석이 있어야 이해되는 코드는 코드를 고친다"** 고
말한다. 전면 정리(PR #269~#271)는 그 축을 건드리지 않았다 — **주석 7,737줄을
한국어로 만들었을 뿐, 주석이 필요한 이유를 줄이지는 않았다.** 작업자들에게
"코드를 고치지는 마라"라고 명시했기 때문이며, 그 판단 자체는 옳았다(한 PR에
번역과 리팩터를 섞으면 검토가 불가능해진다). 남은 절반이 이 항목이다.

`[실측 2026-07-31 · main 538c337]` 프로덕션 파이썬(테스트 제외) 기준,
논리연산·비교가 3개 이상인 조건식 **39곳**. 그중 **6곳은 바로 위 주석이 조건을
설명하고 있다**:

`archive/models.py:324` · `archive/services.py:480` · `core/pagination.py:29` ·
`core/views/archive.py:362` · `core/views/collection.py:115` ·
`drafts/services.py:229`

**가장 명확한 것부터**: 같은 식이 두 파일에 그대로 중복돼 있다.

```python
if not ident.isascii() or not ident.isdigit() or len(ident) > 18:
```
`core/views/archive.py:362`, `core/views/collection.py:280`.
한쪽에는 3줄 주석이 무슨 뜻인지 설명하고 다른 쪽에는 아무 설명이 없다.
이름 하나(예: 안전한 pk 문자열인지 묻는 술어)면 **주석과 중복이 함께 사라진다.**

⚠️ **6곳을 일괄 처리하지 마라.** 성격이 다르다:
- `archive/models.py:324`의 주석은 조건의 *뜻*이 아니라 **왜 이 검사가 여기
  있는가**(다른 테이블을 참조해 DB 제약으로 강제할 수 없다)를 말한다. 그건
  코드가 담을 수 없으니 **남겨야 한다.** 조건식에 이름을 붙이되 주석은 유지.
- 반대로 위 중복 술어의 주석은 조건을 풀어 쓴 것이라 이름이 생기면 불필요해진다.

**범위**: 조건에 이름을 붙이는 것은 코드 변경이므로 백엔드 TDD 대상이다.
술어를 어디에 둘지(모델 메서드 / 도메인 함수 / 모듈 사설 함수)는 소유 도메인을
...
| F6 | ~~SSRF DNS 리바인딩 TOCTOU~~ (**해결됨 2026-08-20**) | `validate_fetch_url`이 검증 IP를 반환하고 `fetch_html`이 그 IP로 직접 연결(Host 헤더·`sni_hostname`으로 원래 호스트 유지), 계약 테스트는 `tests/drafts/test_draft_fetching_ip_pinning.py`, 실수집에서 핀닝 IP로의 TLS 200 실측 | 해결 |
...
⚠️ **필터를 `core/templatetags/`에 두면 Red다.** core 경계 가드가 기본 거부이고
허용 목록은 `core/views/`와 `core/promotion.py` 둘뿐이라, `drafts.labels`를
끌어오는 필터는 `drafts/templatetags/`에 있어야 한다.

★ **모델 `choices` 라벨 한국어화는 다시 기각됐다.** `[실측 2026-08-01]` 바꾸면
`AlterField` 마이그레이션이 생긴다(DB 스키마는 무변경). 어휘 가드 트랙의 기각
근거와 같아 사용자가 앱 상수 방식을 택했다.

★ **선재 갭을 함께 닫았다.** 필터가 틀린 라벨을 내도 2146건이 **전부 통과**했다 —
칩 라벨 렌더를 보는 테스트가 하나도 없었다. 리터럴이던 시절에도 같았으므로
