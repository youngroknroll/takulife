...
| F13 | ~~대시보드 표 저강조 색 위계 미실현~~ (**해결됨 2026-08-25, 브랜치 `fix/media-overwrite-lowrisk-sweep`**) | `.dash-table`로 색 상속 전환(`static/css/staff/pages/dashboard.css`), 라이트·다크 모두 computed 실측으로 `.dash-cell-dim`·`-faint`·`-wrap` 전부 토큰 일치 확인, WED·BIR `Conforms`×2 | 해결 |
| F14 | 스태프 표 CSS 동일 특이도 패턴 잔존 2건 | `static/css/staff/pages/sources.css` `.src-table tbody td`(0,2,1)가 `.src-cell-dim`·`.src-cell-error`(0,1,0)를 이겨 **실패 사유 위험색(`--c-danger`) 신호가 소실**(적용처 `templates/staff/sources/list.html:40,47,48`) / `static/css/staff/pages/audit_log.css:67-71` `.audit-table tbody td`가 `.audit-cell-dim`·`-faint`를 이김(적용처 `templates/staff/audit_log/list.html:49,52`) `[코드 2026-08-25, BIR 전수 grep]` | F13과 동일 수정 방향(표 차원 상속 전환). `.src-cell-error` 쪽이 우선순위 높음(오류 신호 소실) |
