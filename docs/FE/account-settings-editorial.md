...
**해결**: 탈퇴 뷰(GET+POST)를 화면 합성 계층으로 옮겼다. 그 파일은 이미
`archive.queries`를 임포트한다. 락아웃 헬퍼와 상수는 `accounts/services.py`로
승격해 보안 규칙 소유권은 `accounts`에 남겼다.

> **갱신(2026-08-02)**: 그 파일은 이제 `web/views/account.py`다. 착수 당시에는
> 경계 가드가 `core/views/`를 예외로 허용해서 성립하던 배치였는데, 프레젠테이션
> 계층이 `web` 앱으로 분리되면서 그 허용 목록 자체가 없어졌다. 이제는 예외가
> 아니라 **`web`이 도메인을 임포트하는 쪽이라는 설계**로 성립한다.
...
⚠️ **이 진단은 틀렸다** (2026-07-31 정정, PR #266). Django 5.2가 두 속성을 자동으로
붙이고 있었고, 실제 결함은 그 `aria-describedby`가 **문서에 없는 id를 가리켰다**는
것이었다(Django는 `id_x_error`, 파셜은 `id_x-error`). 근거는
`docs/FE/auth-editorial.md` A2. 아래는 정정 전 원문이다.
