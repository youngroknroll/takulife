...
커맨드바 검색창이 무동작이었다. `q`를 읽는 뷰가 없고 바인딩하는 JS도 없었으며,
`action="{{ request.path }}"`라 제출하면 보고 있던 필터가 통째로 날아갔다.
**사용자 결정으로 배선했다**(2026-08-01).

## S8 검색 대상에 무엇을 넣지 않을지가 중요하다

감사 로그 검색은 `ip_address`·`user_agent`를 **대상에 넣지 않는다**. 값을 화면에
보여주지 않아도 검색이 걸리면 「203.0.113.99로 검색해 결과가 나오는가」를 되물어
존재 여부를 알아낼 수 있다 — D9 제한이 무의미해진다.
`tests/staff/test_staff_queries.py::test_ip_주소로는_검색되지_않는다`가 지킨다.

## S9 검색 폼은 보고 있던 상태를 숨은 입력으로 이어받아야 한다

`staff/search.py`의 `search_form_hidden_params`가 `q`·`page`를 뺀 나머지 질의
파라미터를 숨은 입력으로 낸다. 없으면 검색 한 번에 상태 필터·경고 드릴다운이
풀린다. 페이저도 같은 이유로 `q`를 유지한다 — 2쪽으로 넘기는 순간 검색이 풀린다.

⚠️ **빈 상태 문구에 조사를 쓰지 마라.** 「"{{ search }}"와 맞는」은 검색어 끝
글자가 자음이면 「과」여야 한다. 값이 사용자 입력이라 템플릿에서 고를 수 없어
조사 없는 문형(「검색 결과가 없습니다」)을 쓴다.
...
## S11 단언은 결함이 드러나는 상태를 실제로 태워야 한다

트랙 26(정렬·기간·카테고리 필터)에서 같은 뿌리로 결함 4건이 났다.

① 도메인 테스트가 전부 `today=`를 명시로 넘겨 기본값 해석 경로를 한 번도
안 태웠다 — 웹으로 `?period=`·`?sort=`를 기본값째 요청하자 500
(`ValueError: Cannot use None as a query value`).

② 링크 테스트가 부분 문자열로 앵커를 골랐는데, 모든 링크가 모든 축을
이어받게 되자 marker가 어느 그룹의 링크인지 특정하지 못했다 — 뮤테이션을
넣어도 통과했다(아무것도 증명하지 못한 단언).

③ 기존 게시상태 탭 테스트가 `.events-tab`을 문서 전체에서 세었는데,
정렬 탭이 같은 클래스를 재사용하면서 개수가 어긋나 깨졌다(의도된 클래스
공유였는데 테스트가 그 설계를 몰랐다).

④ 카테고리 칩 합계 테스트가 카테고리 **미선택** 상태만 검사해, 카테고리를
**선택한** 상태에서만 나는 결함(칩 합계와 표 하단 건수가 기준이 다른
숫자라는 것, `docs/BE/staff-event-publish-actions.md` (q))을 놓쳤다.

대응: 링크·칩 단언은 `aria-label`로 그룹 구간을 먼저 자른 뒤 그 안에서
찾는다. 그리고 **컨텍스트 값만 단언하면 템플릿이 다른 소스를 써도
통과한다** — 렌더된 HTML의 실제 숫자·텍스트를 봐야 한다(④가 정확히
그 경우였다: `category_chip_total`이 아니라 칩에 렌더된 `(N)` 텍스트를
비교해야 결함이 드러났다). S9(검색어 단언 함정)와 같은 계열이다.
