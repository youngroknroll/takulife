...
순수히 **줄 수 예산** 문제다. 아래 한 줄 요약 목록은 PR #282부터 PR #149까지를 담았다.
...
### PR #286 — feat(web): 홈 히어로 타이포 자동 슬라이드 로테이터

**무엇을 바꿨나**: D4 포스터 제거로 사라진 카드스택을 대체해 홈 히어로
대표 행사를 타이포그래피 자동 슬라이드로 확장. 뷰 컨텍스트를
`featured_event_row`(단수)에서 `featured_event_rows`(진행중→예정 우선순위
최대 5건 리스트)로 전환하고, 템플릿을 0건=빈 상태 / 1건=컨트롤 없는 정적
표제 / 2건+=로테이터로 분기. grid 겹침 스택 + visibility 지연 크로스페이드
(7초)로 높이·CTA 좌표를 고정하고, 도트(44×44, 활성=색+폭 이중 표시)와
정지/재생 토글은 신규 `static/js/pages/home_hero_rotator.js`가 주입한다
(no-JS는 첫 슬라이드 정적, 죽은 컨트롤 없음). hover/focus 정지, 도트
클릭=영구 정지(WCAG 2.2.2), reduced-motion이면 자동 회전·토글 미렌더.

**왜**: 2026-08-16 사용자 요청. 포스터 없는 히어로가 단건 고정이라 노출
행사가 하나뿐이었다.

**검증**: 전체 회귀 2148 → 2149 passed `[실측]`(신규 1: 최대 5건·우선순위).
check 0 issues, 드리프트 없음 `[실측]`. FE 이중 게이트 사전 명세 + 사후
판정 — WED `Conforms`, BIR 최초 `Deviates`(visibility 지연 350ms 동안
이중 tabbable 창, 코드 확정) → 전환 틱 `inert` 부여로 수정·재실측 후
`Conforms`. Chromium 실측 14개 시나리오(주기 6999ms, reduce 왕복, AX
트리, no-JS, 320px, 다크·라이트). bfcache `pageshow` 분기만 DevTools
부착 한계로 코드 검토 갈음(`docs/FE/event-typography-editorial.md`
Known gap 기록).

**병합**: 2026-08-16, main `6d70e6e`. 같은 날 #285(브라우저 검토 후속
1줄 수정 3건: "위시"→"찜" 어휘 통일, 히어로 제목·상세 기간 keep-all)도
머지.
