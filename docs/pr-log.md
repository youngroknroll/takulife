...
"재구성 불가"가 아니라 순수히 **줄 수 예산** 문제다. 아래 목록은 PR #308부터 #167까지다.
...
### PR #309 — fix: 전수 검토 잔여 프론트 3건 반영 (500 헤더 착시·필수 표시 통일·죽은 캐러셀 제거)

**무엇을 바꿨나**: 전수 검토 잔여 프론트 3건. ①500 페이지가 `site_header`
블록 오버라이드로 인증 영역 없는 중립 헤더를 렌더 — Django `server_error()`가
request 없이 렌더해 로그인 사용자에게도 "로그인/회원가입"이 보이던 착시
해소(공용 셸 `_site_header.html`·`_topbar.html`·`base.html` 무수정).
②인증·계정설정 8화면의 필수(＊) 표시를 archive 폼과 동일한 aria-hidden
별표 + sr-only "(필수)" + 폼 상단 범례 패턴으로 통일, 전역 `.sr-only`
부재라 `auth.css`·`account_settings.css`에 페이지 스코프 clip 규칙 동반
신설. ③`visit.js`의 죽은 사진 캐러셀 핸들러를 정의·호출부 2곳 동시
제거(호출부 잔존 시 삭제 버튼 바인딩 마비 위험을 사전 기준이 지목).

**왜**: 전수 검토(2026-08-24) 잔여 항목 사용자 지시("③부터 정리"). FE
이중 게이트 Standard 완주 — WED·BIR 사전 명세/기준 → 구현 → 사후 판정
**Conforms/Conforms**.

**검증**: 브라우저 실측(DEBUG=false+collectstatic 서버, 임시 500 유발
경로는 측정 후 원복) — 로그인 상태 실제 500에서 헤더 인증 링크·계정
메뉴 전무, 404는 로그인 반영 유지, sr-only 전 지점 1×1px·
aria-describedby 끊김 0·320px 무오버플로, 삭제 흐름 DELETE 204·검색
교체 후 재바인딩 정상, 콘솔 JS 오류 0 `[실측]`. 전체 회귀 **2282
passed**(프론트 전용, 건수 불변) `[실측]`. CI 4잡 pass.

**병합**: 2026-08-24, main `25d93c7`.
