...
행사 등록·검수 운영 기준. `.docs/plans/2026-07-14-stage0-deployment-foundation-plan.md`
(이하 "0단계 계획서") T8, 기획서 §14 행사 공급 절을 기준으로, 실제 드래프트
검수 화면·모델 필드와 대조해 작성. LLM 자동 승인·발견은 이 문서 작성 시점
기준 비용 정책상 비활성 상태다(아래 §4).
...
- 초안 파이프라인은 `create_draft_from_url`에서
  `settings.DRAFT_LLM_EXTRACTION_ENABLED`(`config/settings.py:245`, 현재
  **`False`로 고정**)가 켜진 경우에만 LLM 추출(`extract_event_fields_llm`)을
  사용하고, 꺼져 있으면 휴리스틱 추출로 폴백한다(`drafts/services.py:99`).
