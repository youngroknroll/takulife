...
- **만료 행사 일괄 비공개 처리(트랙 24, H2)**: `/staff/events/`에서 경고 필터 "종료됐지만 게시 중"을 걸고 체크박스로 대상을 선택한 뒤 "선택 항목 비공개로 설정"을 누르면 한 번에 최대 20건[코드]까지 처리된다(`POST /staff/events/bulk-unpublish/`, 상세 가드레일은 `docs/BE/staff-event-publish-actions.md`가 정본). 부분 실패 시 처리되지 않은 행에는 사유가 표시되며, 서버가 멱등이라 같은 선택으로 재시도해도 이미 처리된 항목이 다시 바뀌지 않는다.
- **이벤트 목록 행 인라인 처리(트랙 25, H2)**: 목록 행에서 비공개·다시 게시·검증 완료를 바로 처리할 수 있고, 검증 완료는 되돌릴 수 없어 확인 모달을 거친다.
...
- 스키마(테이블) 자체를 제거해야 하는 경우는 별도 승인과 보존 정책이 먼저 필요하다(2026-07-19 이중 달력 서비스 설계서 §14 결정 — 원문서는 2026-08-29 문서 정리로 삭제).
...
- **env 파일**: 러너 시크릿(`DRAFT_DISCOVERY_RUNNER_TOKEN`,
  ...
  ~/.takulife-runner.env; set +a`로 셸에 로드한 뒤 `uv run python -m
  local_runner.runner`로 기동한다. 토큰 값은 이 문서를 포함해 어떤
  문서에도 적지 않는다.
- **서버 주소는 반드시 `https://takulife.kr`(www 없이)**. `www.takulife.kr`은
  `takulife.kr`로 301/307 리다이렉트되는데, 러너 클라이언트는 이
  리다이렉트를 따라가지 않아 첫 heartbeat부터 실패한다 `[실측
  2026-09-20]`.
- **정상 종료는 Ctrl+C 한 번만. `kill -TERM <pid>`도 같은 경로다** —
  러너는 SIGTERM을 SIGINT와 동일하게 `KeyboardInterrupt`로 받는다(배포
  재시작이 보내는 신호도 이 경로를 탄다). 첫 신호에서 러너가 진행
  중이던 run이 있으면 `runner_shutdown` 사유로 완료 보고를 먼저 보내고
  이어서 오프라인 보고를 보낸다 — 대시보드 오프라인 배지는 다음 폴링
  주기 이내(목표 5초 이하)에 반영된다. 탐색·읽기·제출 도중 종료하면 그
  실행은 즉시 `실패(runner_shutdown)`로 표시되며 서버·러너 어느 쪽도
  자동 재시도하지 않는다 — 같은 검색어를 다시 돌리려면 스태프가
  대시보드에서 재탐색을 다시 요청해야 한다. **두 번째 Ctrl+C는 오프라인
  보고를 건너뛰고 즉시 종료한다** — 이 경우 대시보드는 기존 120초
  신선도 만료로만 오프라인을 알게 된다. 실기동 실측(2026-09-20,
  로컬 개발 DB): 대기 중 SIGTERM은 신호→오프라인 보고까지 31ms, 대기
  중 SIGINT는 27ms, 탐색 중 SIGINT는 완료 보고 324ms→오프라인 보고
  363ms.
- **토큰 회전 절차**: (1) Render env `DRAFT_DISCOVERY_RUNNER_TOKEN`을
  새 값으로 교체, (2) 로컬 `~/.takulife-runner.env`도 같은 새 값으로
  교체, (3) 서버 컨테이너 재기동(env는 프로세스 기동 시 1회만
  평가된다 — §8 "활성화" 항목과 같은 제약), (4) 러너 재시작. 순서가
  바뀌면(러너를 먼저 새 토큰으로 바꾸면) 서버가 아직 옛 토큰이라
  러너 API 전부가 403을 반환한다.
- **터미널 로그 읽는 법**: 러너가 남기는 한국어 진행 줄은 상태 전이
  시점에만 찍히며, 대략 다음 순서로 나타난다 — "서버 응답 확인 —
  러너 온라인" → `검색어 "…" 검색 중` → `행사 확인 중 (i/n)` — 있으면
  ` — {host}` 덧붙음 → `소스 후보 제출 중 (j/m)` → `완료 보고 정리
  중` → `대기` → (다음 폴에서 다시) `서버 응답 확인 — 러너 온라인` →
  (종료 시) `러너 종료 — 오프라인 보고 완료`. 대기 중 폴링 실패·복구도
  상태가 바뀔 때만 1줄 남는다(매 폴마다 찍지 않는다). 실기동
  실측(2026-09-20, 실행 19, 검색 결과 행사 0건)으로 확인한 순서는
  `검색어 "…" 검색 중` → `행사 확인 중 (0/0)` → `완료 보고 정리 중` →
  (당시 대기 전이 줄이 빈 채로 찍히던 결함, 같은 트랙 커밋 `bc65163e`로
  수정 — 지금은 이 자리에 `대기`가 찍힌다, 회귀
  `tests/local_runner/test_progress.py` 확인) → `서버 응답 확인 —
  러너 온라인`.
