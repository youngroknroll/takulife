...
일부러 ``core``에 둔다: ``archive``는 ``drafts``를 임포트할 수 없고
반대도 마찬가지라, 개인 archive 항목을 검토 드래프트로 연결할 수 있는
유일한 곳이 이 중립 레이어다. 사용자의 PersonalEntry(archive)를 읽어
그 값으로 EventDraft(drafts)를 만들고, 항목을 제출됨으로 표시한다.
